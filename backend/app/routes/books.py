...
def get_books(
    ...
    return (
        db.query(models.Book)
          .filter(models.Book.owner_id == current_user.id, models.Book.archived == False)
          .all()
    )
...
def get_archived_books(
    ...
    return (
        db.query(models.Book)
          .filter(models.Book.owner_id == current_user.id, models.Book.archived == True)
          .all()
    )
...
def get_stats(
    ...
    for b in books:
        ...
        pct = round((b.pages_read / b.total_pages) * 100, 2) if b.total_pages else 0
        book_progress.append({"id": b.id, "title": b.title, "percent_completed": pct})
